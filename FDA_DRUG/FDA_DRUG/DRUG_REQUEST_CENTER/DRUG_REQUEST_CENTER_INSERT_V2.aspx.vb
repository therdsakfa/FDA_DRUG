Public Class DRUG_REQUEST_CENTER_INSERT_V2
    ...
    Protected Sub btn__Click(sender As Object, e As EventArgs) Handles btn_.Click
        hf_refno.Value = ""
        lbl_result.Text = ""
        Dim results As Integer = 0
        Dim i As Integer = 0
        Dim j As Integer = 0
        Dim dao_p As New DAO_SP_PAYMENT.TB_PAYMENT_DETAIL
        i = dao_p.count_REFNO(txt_refno.Text)
        dao_p = New DAO_SP_PAYMENT.TB_PAYMENT_DETAIL
        dao_p.GetDataby_REFNO(txt_refno.Text)
        Dim dao_sp As New DAO_SP_PAYMENT.TB_SYSTEMS_PAYMENT_DETAIL
        j = dao_sp.count_REFNO(txt_refno.Text)

        dao_sp = New DAO_SP_PAYMENT.TB_SYSTEMS_PAYMENT_DETAIL
        dao_sp.GetDataby_REFNO(txt_refno.Text)

        results = i + j
        Dim ref_no As String = ""
        Try
            If results <> 0 Then
                ref_no = dao_p.fields.REF_NO
                Try
                    If dao_sp.fields.IDA <> 0 Then
                        hf_refno.Value = dao_sp.fields.REF_NO
                    End If
                    If dao_p.fields.IDA <> 0 Then
                        hf_refno.Value = dao_p.fields.REF_NO
                    End If
                Catch ex As Exception
    ...
    Public Sub set_data(ByRef dao As DAO_DRUG.TB_DRUG_REQUEST_CENTER)
        ...
        Try
            dao.fields.PROCESS_DIGITS = hf_refno.Value
        Catch ex As Exception

        End Try
        Try
            dao.fields.REF_NO = hf_refno.Value
        Catch ex As Exception
    ...
    Private Sub btn_add_Click(sender As Object, e As EventArgs) Handles btn_add.Click
        If hf_refno.Value <> "" Then
            ...
            End If
        Else
            ERRORalert("กรุณาตรวจสอบเลขอ้างอิง")
        End If
    ...
    End Sub
    Sub ERRORalert(ByVal text As String)
